...
def group6(prepped_files):
    """Group files according to timestamp, in preparation for DEM inversion.
    Args:
        prepped_files (iterable): AIA-prepped files to group
    Returns:
        Iterable with name and dataframe containing group details such as time, filename, and wavelength
    """
...
def check_datanumbers(prepped_files):
    """
    Check that data numbers in AIA files have been corrected for exposure time.
    Args:
        prepped_files (iterable): AIA-prepped files to group
    Returns:
        Plotly figure
...
def interp_tresp(trmatrix, logt_in,logt_out):
    """Interpolate temperature response matrix onto new log T vector.
    
    Args:
        trmatrix (np.array): Input temperature response matrix
        logt_in (np.array): Input vector of log10 temperatures corresponding to the matrix
        logt_out (np.array): Vector of log10 temperatures corresponding to the desired response matrix.
        
    Returns:
        np.array : the temperature response matrix interpolated to the new values."""
...
def calc_temp_vars(temps=False,tstart=5.6,tend=6.8,num=42):
    """Calculate temperatures on a log scale, difference between the temperatuers and mean temperatures in bins.
    
    Args:
        temps (np.array, list, or bool, optional): Input temperature bin edges, or False. Defaults to False.
        tstart (float, optional): If temps is not a list or array, this is the start of the log-spaced temperature vector to be generated.
        tend (float, optional): If temps is not a list or array, this is the end of the log-spaced temperature vector to be generated.
        num (int, optional): The size of the temperature vector to be generated.
    
    Returns:
        tuple : temperature bin edges, bin widths, and means
    """
...
def tresp_matrix_from_IDL(datestr):
    """ Run aia_get_response.pro for given date and retrieve the output.
    
    Args:
        datestr (str): String of desired date for which to calculate response function.
        
    Returns:
        tuple : temperature response matrix, temperature axis in log10, units of the temperature response"""
    import pidly
