...
class _LinearView(object):
    ...
    def _find(self, arr, item):
        ...
        for n, mid in enumerate(self.midpoints[min_mid:]):
            if mid <= freq:
                foo=arr[min_mid + n] #added this
                #print(len(foo))
                return foo# arr[min_mid + n]
        #print(len(arr[min_mid]))
        #print(len(self.midpoints[min_mid:]))
        return arr[min_mid]
...
class TimeEnergy(object): #EL copied from class TimeFreq. Should I add a function to log-scale the axis?
    """Class to use for plotting energy vs time.
    Attributes
    ----------
    start : `datetime.datetime`
        Start time of the plot.
...
class Spectrogram(Parent):
    ...
    def __init__(self, data, time_axis, freq_axis, start, end, t_init=None,
                 t_label="Time", f_label="Frequency", c_label="Intensity", content="",
                 instruments=None):
...
#        else:
        ...
        #print(type(data.data),np.shape(data.data))
...
#            pass
#        else:
        toplot = masked_array#data
        im = axes.imshow(toplot, **params)

        xa = axes.get_xaxis()
        ya = axes.get_yaxis()

        #EL format the y-axis to give tickmarks where my energy bins are
        ya.set_ticks([3,11,13,16,20,29])
        ya.set_ticklabels(['4','9','12','18','30','80'])
...
#        )
        ...
        figure.suptitle(' '.join([ get_day(self.start).strftime("%d %b %Y"),'RHESSI Count Flux Spectrogram']))
        ...
        if colorbar:
            if len(figure.axes) > 1:
                Colorbar(figure.axes[1], im).set_label(self.c_label)
            else:
                figure.colorbar(im).set_label(self.c_label)
