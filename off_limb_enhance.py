...
#def files2maps(ff):
#    maps=sunpy.maps.Map(ff)
#    return maps

def crop_arcsec(map,cropx,cropy,plot=False):
    top_right = SkyCoord(cropx[1]*u.arcsec, cropy[1]* u.arcsec, frame=swap_map.coordinate_frame)
    bottom_left = SkyCoord(cropx[0] * u.arcsec, cropy[0] * u.arcsec, frame=swap_map.coordinate_frame)
    submap = swap_map.submap(bottom_left, top_right)
    if plot:
        submap.peek(draw_limb=True, draw_grid=True)
    return submap

def cube2fits(filelist):
    '''Take multiple fits files and store in a single image cube for import into jHelioviewer'''

def off_limb_enhance(aia,plot=False,ret=True):
    ###############################################################################
