...
import sys
sys.path.append('/home/erica/STIX/stixdcpy')
...
def get_stix_flares(start_time,end_time,sortedby='time',lighttime_correct=True,local_copy=True):
    ...
    if lighttime_correct:
        ...
        #df['peak_utc_corrected']=[spacecraft_to_earth_time(d) for d in df.peak_utc]
    if local_copy:
...
def prep_and_fit_data(current_data_file='/home/erica/STIX/data/stix_hek_full.json',timerange=10,threshold=None):
    ...
    df=pd.read_json(current_data_file)
    for k in ['peak_utc','peak_utc_corrected','date_obs','event_peaktime']:
        df[k]=pd.to_datetime(df[k],unit='ms')
    #df['event_peaktime']=[pd.to_datetime(d,unit=ms) if type(d) == int  else pd.to_datetime(d) for d in df.event_peaktime]
    ...
    dfgc.drop_duplicates(subset='_id',inplace=True)
    
    if threshold:
        dfgc=dfgc[dfgc.peak_counts > threshold]
    
    dfit=STIX_GOES_fit(dfgc)
    dfit.do_fit(dfgc)
    dfit.bin_residuals(dfgc)
    dfit.bins=[dfit.bins]
    
    pd.DataFrame(dfit.__dict__,index=[0]).to_json(f"/home/erica/STIX/data/fit_result_{dt.now().date()}.json")
    return dfgc,dfit        
